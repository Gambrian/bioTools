...
screen_dir = setting.screen_dir + "screen.png"
...
class dingDing:
    def __init__(self, adbPath, device):
        self.directory = device
        # 点亮屏幕
        self.adbpower = '"%s\\adb" -s %s shell input keyevent 26' % (adbPath, device)
        # 滑屏解锁
        # self.adbclear = '"%s\\adb" shell input swipe %s' % (directory, config.light_position)
        # 启动钉钉应用
        self.adbopen_dingding = '"%s\\adb" -s %s  shell monkey -p com.alibaba.android.rimet -c android.intent.category.LAUNCHER 1' % (
            adbPath, device)
        # 关闭钉钉
        self.adbkill_dingding = '"%s\\adb" -s %s  shell am force-stop com.alibaba.android.rimet' % (adbPath, device)
        # 返回桌面
        self.adbback_index = '"%s\\adb" -s %s  shell input keyevent 3' % (adbPath, device)
        # 点击工作
        self.adbselect_work = '"%s\\adb" -s %s  shell input tap %s' % (adbPath, device, setting.work_position)
        # 点击考勤打卡
        self.adbselect_playcard = '"%s\\adb" -s %s  shell input tap %s' % (adbPath, device, setting.check_position)
        # 点击下班打卡
        self.adbclick_playcard = '"%s\\adb" -s %s  shell input tap %s' % (adbPath, device, setting.play_position)
        # 设备截屏保存到sdcard
        self.adbscreencap = '"%s\\adb" -s %s  shell screencap -p sdcard/screen.png' % (adbPath, device)
        # 传送到计算机
        self.adbpull = '"%s\\adb" -s %s  pull sdcard/screen.png %s' % (adbPath, device, screen_dir)
        # 删除设备截屏
        self.adbrm_screencap = '"%s\\adb" -s %s  shell rm -r sdcard/screen.png' % (adbPath, device)

        # print(self.adbpower)
        # print(self.adbopen_dingding)
        # print(self.adbkill_dingding)
...
def job1():
    print("开始打卡")
    dingDing(directory, "WJX5T17402013756").goto_work()


def job2():
    print("开始打卡")
    dingDing(directory, "WJX5T17402013756").off_work()
